...
- **`generateQuiz.ts`** — `generateQuiz({apiKey, dayNumber, lang, count?})`
  ...
  via the `offlinePack.v4` `localStorage` key.
...
cached to the same `offlinePack.v4` slot as a real Gemini pack —
...
- **Offline pack: indefinite `localStorage` cache** keyed
  `…quiz.offlinePack.v4.day{N}.{lang}`. No TTL — questions about a
...
| Storage | `tuscany2026.quiz.offlinePack.v4.day{N}.{lang}` (no expiry) | none — every batch fresh |
...
| "New questions" on score screen | Clears `offlinePack.v4`, regenerates 10 | Just kicks off a fresh first batch |
...
| Offline pack storage key | `tuscany2026.quiz.offlinePack.v4.day{N}.{lang}` (no TTL — used for both real Gemini packs and template fallbacks) |
